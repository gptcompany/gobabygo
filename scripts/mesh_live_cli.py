    ...
    return "\n".join(
        [
            ...
            *ensure_control_lines,
            ...
            "",
            *speckit_runtime_policy,
            "Spec Kit execution boundary (policy, not runtime attestation):",
            "Use enabled Spec Kit skills and spec/plan/tasks artifacts; Mesh owns managed worker dispatch and receipts.",
            "Do not run `specify workflow run` or `specify workflow resume` to delegate production work. "
            "Native command/prompt steps can launch unmanaged AI CLIs, and native resume can replay side effects.",
            "The canonical Mesh workflow is coordination policy, not authorization to start a second workflow executor.",
            "Use the existing review dispatch path for ledger-managed corrections. "
            "Reconcile unknown delivery receipts before any retry; never replay a task because a workflow step failed.",
            "Installed phases and version alignment do not prove a feature ledger is initialized or GitHub sync is ready. "
            "Check their actual state separately; never silently replace a missing ledger with manual review counts.",
            "These instructions do not sandbox raw CLI access. Attaching to an existing session does not reload this contract; "
            "a fresh managed bootstrap or resume is required to load updated instructions.",
            "",
            *workflow_policy,
            ...
            f"- Initialize a frozen task cycle with `{speckit_review_command} init`; open each review with `{speckit_review_command} open`, then require `{speckit_review_command} ack` from the exact reviewer session and delegation ID with a redacted feature-local evidence file before recording a verdict. An open review is `REVIEW_PENDING_ACK` for five minutes; only ACK starts the normal review deadline. Persist the exact non-secret reviewer report inside the feature and record it with `{speckit_review_command} record --evidence-file <report>`. The CLI computes the digest and rejects mutable scope, self-review, blocking PASS, budget overflow, duplicate review, or invalid transition.",
            f"- `status` exposes either the pending ACK or active review deadline. Timeout is never consent: only after that deadline and without a valid final marker may you run `{speckit_review_command} timeout` with the current revision. An ACK timeout and a review timeout share one fallback on a different reviewer session for the same scope; a second failure transaction escalates. Never resend to the timed-out reviewer or create an open-ended reviewer loop. Before the deadline, `{speckit_review_command} abandon` is allowed only for DELIVERY_UNVERIFIED, AUTH_SESSION_INVALID, REVIEWER_UNAVAILABLE, or HEARTBEAT_EXPIRED with a redacted evidence file inside the feature and the current revision; it consumes that same fallback budget. Never infer abandonment from pane prose or capture output.",
            ...
            "- After REPLAN, restarting the same canonical task requires `init --replan-file <feature-relative-json>`. The JSON must contain task_key, previous_cycle, failure_evidence, approach_change, acceptance_criteria, and finding_disposition. Inspect carried findings and total_correction_rounds before restarting. A changed task name is not a replan; preserve the canonical parent identity. The artifact records your reasoning, not proof that the new strategy is correct.",
            "- Legacy subtask names require an explicit `review map <repo> <feature> <Tnnn> --alias <legacy-name> --evidence-file <mapping-report> --expect-revision <revision>`. Use `review resolve` to obtain the canonical task before further transactions. Aliases share one cycle and cannot be reassigned. Missing github-ledger.json or canonical tasks is a migration blocker, never permission to claim manual counts are transactionally gated.",
            ...
            f"- Deliver planned corrections with `{speckit_review_command} dispatch <repo-root> <feature-dir> <Tnnn> --worker-repo <writer-repository> --delegation-id <open-correction-id> --message <single-line-task> --expect-revision <revision> --json` on the host holding both ledger and tmux. Include the canonical task key and delegation ID in the message. This uses the existing tracked live send, records intent before input and refuses replay even after a lost response. On unknown delivery, inspect the recorded target; use only existing guarded recovery, never raw resend or a new ID for the same task. After exact RELEASE PASS use `review complete` with the current scope and revision to update the canonical checkbox, then let GitHub Action reconciliation run. Do not edit planned completion checkboxes manually.",
