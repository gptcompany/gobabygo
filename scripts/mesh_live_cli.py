    ...
    return "\n".join(
        [
            ...
            *workflow_policy,
            ...
            f"- Never send a planned correction unless the transaction returns `CORRECTION_OPEN`. Before checking the authoritative task complete, require `{speckit_review_command} check <repo-root> <feature-dir> <Tnnn>` to exit 0 with `RELEASE_PASSED`. RELEASE_PASSED remains evidence, not deploy or money-path authority.",
