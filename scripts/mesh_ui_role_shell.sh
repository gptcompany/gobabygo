...
if is_local_ws_host "$WS_HOST"; then
  ...
  bootstrap_shell "$TARGET_DIR" "$WS_REPO_BASE" "$ROLE" "$REPO_NAME" "$REMOTE_INIT" "$LIVE_ATTACH_MODE" "$UI_GROUP_ID"
fi
...
REMOTE_COMMAND="$(printf 'TARGET_DIR=%q WS_REPO_BASE=%q ROLE=%q REPO_NAME=%q ROLE_SET=%q REMOTE_INIT_B64=%q LIVE_ATTACH_MODE=%q UI_GROUP_ID=%q bash -lc %q' \
  "$TARGET_DIR" "$WS_REPO_BASE" "$ROLE" "$REPO_NAME" "$ROLE_SET" "$REMOTE_INIT_B64" "$LIVE_ATTACH_MODE" "$UI_GROUP_ID" "$REMOTE_BOOTSTRAP_SCRIPT")"
