...
- [x] T032: Keep Codex and Antigravity rate limits fail-closed until each provider exposes an exact, tested reset contract. Report a provider-specific blocker, allow an explicitly declared worker substitution, and forbid guessed wake times, blind Enter, task resend, or automatic session replacement.
...
- [x] T036: Project the exact matching persisted Claude schedule into dry-run, supervisor, and apply output instead of recalculating a passed reset banner. Reject malformed persisted timestamps, independently review the fix, and verify on the Dell that the reported `not_before` remains canonical after the guarded wake.
- [x] T037: Recover an unchanged pending Claude composer after an unverified post-reset Enter without leaving the coordinator blocked indefinitely. Permit at most three total Enter-only attempts for the same pane/banner/composer fingerprint, separated by at least four minutes (normally the next five-minute managed tick) and tombstoned before input; recapture every time, never resend text, and retain one-shot behavior for an empty composer. Cover persistence, backoff, exhaustion, fingerprint changes, review, and a Dell E2E.
