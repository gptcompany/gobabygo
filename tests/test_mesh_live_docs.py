...
def test_speckit_migration_docs_include_single_owner_and_legacy_policy() -> None:
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    quickstart = (ROOT / "QUICKSTART.md").read_text(encoding="utf-8")
    runbook = (ROOT / "MESH_LIVE.md").read_text(encoding="utf-8")

    assert "mesh speckit project migrate /data/sata/1TB/rektslug" in readme
    assert "mesh speckit project init /data/sata/1TB/rektslug" in quickstart
    assert (
...
def test_mesh_live_runbook_covers_operator_contract() -> None:
    ...
    for required in (
        ...
        "exact resolved Git root and provider process, never only a session name",
        "--role reviewer",
        "independent-reviewer-exemption",
        "never terminates, adopts, or renames a worker",
        ...
        "`mesh live retire` is the only Mesh Live termination path",
        "never invoked by `tick`",
        "--apply --confirm codex-old",
        "a failed attempt is durable and is never retried automatically",
        "A single guarded recovery is available only",
        "recent matching tracked-delivery receipt",
        "submission=verified",
        ...
        "cron recovery requires explicit installer",
        "tick --apply --recover-coordinator",
        "iTerm2 local state",
