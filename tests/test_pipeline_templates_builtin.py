...
def test_canonical_templates_are_session_only() -> None:
    ...
    for name in ("confidence_gate_team", "gsd", "speckit", "speckit_codex", "antigravity_team_demo"):
...
def test_gsd_and_speckit_are_multi_model_team_templates() -> None:
    ...
    for name in ("gsd", "speckit"):
        ...
        assert {"claude", "codex", "antigravity"} <= clis
...
def test_antigravity_team_demo_is_antigravity_only() -> None:
    ...
    assert steps
    assert {str(step.get("target_cli", "")).strip() for step in steps} == {"antigravity"}
    assert {str(step.get("role", "")).strip() for step in steps} == {"president", "lead", "worker"}


def test_antigravity_team_demo_defines_file_based_auto_exit() -> None:
    templates = _load_templates()
    steps = templates["antigravity_team_demo"]["steps"]
    expected = {
        "lead_plan.md": "ANTIGRAVITY_LEAD_OK",
        "worker_review.md": "ANTIGRAVITY_WORKER_OK",
        "president_decision.md": "ANTIGRAVITY_TEAM_OK",
    }
    seen: dict[str, str] = {}
    for step in steps:
