...
Enforces strict BOSS -> PRESIDENT -> WORKERS -> PRESIDENT communication
hierarchy. Every operation that violates the hierarchy is a hard block.
...
# Only president dispatches tasks to workers.
_DISPATCHERS: set[str] = {CommunicationRole.president.value}
...
class CommunicationPolicy:
    ...
    def can_create_task(self, creator_role: str) -> bool:
        """Only boss/president can create tasks. Workers cannot."""
        return creator_role in _TASK_CREATORS

    def can_dispatch_task(self, dispatcher_role: str) -> bool:
        """Only president role dispatches tasks to workers."""
    ...
    def can_view_all_tasks(self, role: str) -> bool:
        """Boss/president can view all tasks. Workers have scoped visibility."""
